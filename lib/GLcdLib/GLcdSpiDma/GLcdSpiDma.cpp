...
//SPIバスにLCDモジュールを追加する
bool	GLcdSpiDma::AddDevice(uint8_t spiMode, int32_t busSpeedHz, gpio_num_t pinSS, gpio_num_t pinDC)
{
	//ピンモードを設定する
	SetPinMode(pinSS, pinDC);
	if (IsValidGpio(pinSS)) { gpio_set_level(pinSS, 1); }
	if (IsValidGpio(pinDC)) { gpio_set_level(pinDC, 1); }
	this->pinDC = pinDC;

	//SPIバスを初期化する（DMA対応メモリが確保される）
	spiDma.Initialize();

	//SPIバスにデバイスを追加する
	spi_device_interface_config_t spiDevIfConfig;
	SetDeviceConfig(spiMode, busSpeedHz, pinSS, &spiDevIfConfig);
	bool isOK = spiDma.AddDeviceToBus(&spiDevIfConfig, &devHandle);
	return isOK;
}
...
void	GLcdSpiDma::SetPinMode(gpio_num_t pinSS, gpio_num_t pinDC)
{
	...
	pinConfig.pin_bit_mask = 0;
	if (IsValidGpio(pinSS)) { pinConfig.pin_bit_mask |= (1ULL << pinSS); }
	if (IsValidGpio(pinDC)) { pinConfig.pin_bit_mask |= (1ULL << pinDC); }
	if (pinConfig.pin_bit_mask != 0)
	{
...
void	GLcdSpiDma::SetDeviceConfig(uint8_t spiMode, int32_t busSpeedHz, gpio_num_t pinSS, spi_device_interface_config_t* devConfig)
{
	...
	devConfig->clock_speed_hz = busSpeedHz;	//80MHzを分周した値が最適（SPI_MASTER_FREQ_xxMが使える）
	devConfig->spics_io_num = pinSS;	//SPIのSSピン（使用しないなら'-1'）
...
void	GLcdSpiDma::SendData(const uint8_t* datas, size_t length) const
{
	ChangeDataMode();
	if (spiDma.BufferSize() < length)
